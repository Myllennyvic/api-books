...
function renderizarTitulo(items) {
  ...
  container.innerHTML = ''; // Limpar o container antes de renderizar novos títulos
  items.results.forEach((item, index) => {
      const divTitulos = document.createElement('div');
      const price = getRandomPrice(15, 150); // Geração de preço aleatório
          ...
          <div class="titulo-caixa">
              ...
              <div>
                  ...
                  <h3 class="title">R$ ${price}</h3>
                    <button class="bnt" onclick="window.location.href = './pages/infoBooks.html?=${index}'">Descrição</button>
              </div>
          </div>
      `;
